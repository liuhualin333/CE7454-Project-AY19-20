...
config = 'configs/hr32.py'
checkpoint = 'work_dirs/hr32/epoch_12.pth'
input_image =  '../data/train_images/200014740-00024_1.jpg'
#input_image =  '../data/test_file2/200014740-00024_1.jpg'
...
def extract_sequence(cluster_idxs, bboxes_center, labels):
    ...
    for column_num in column_order:
        ...
        newclust_dict[column_num]=cluster_dict[column_num]
            ...
            if len(last_elem) != 0:
                ...
                if dist < 1:
                    if len(proposed_proposals) == 0:
                        ...
                        all_elems[-1] = label # Append the character
                    ...
                    proposed_proposals = []
                    all_elems.append(label) # Append the character
                else:
                    all_elems.append(label) # Append the character
                # Post-process
                ...
                all_elems.append(label) # Append the character
            last_elem = elem[:2]
    cluster_dict=newclust_dict
    #print(f'Column and characters in order: {newclust_dict}')
...
def kmean_find(data,n_clust,centroids,minx,miny,maxiter):
    s=dict();ss=[];ssss=[];sclust=[]
    for i,cen in enumerate(centroids):
      s.update({i:[cen[0], cen[1]]})
    a=sorted(s.items(), key=lambda x: x[1])
    for _,(_,v) in enumerate(a):
      ss.append(v)
    for i,v in enumerate(ss):
      ...
      xd=minx
      yd=miny
      box0=box(x-xd,y-yd,x+xd,y+yd)
      if ss.pop(i)==None:
        print("error")
      for _,w in enumerate(ss):
        x2=w[0]
        y2=w[1]
        box1=box(x2-xd,y2-yd,x2+xd,y2+yd)
        if box0.intersects(box1):
          ssss.append(w)
    [sclust.append(x) for x in ssss if x not in sclust]
    #print(sclust)
    #print("best n_clust:", n_clust-len(sclust))
    n_clust=n_clust-len(sclust)
    kmeans = KMeans(init='k-means++', max_iter=maxiter, n_clusters=n_clust, n_init=10)
    kmeans.fit(data)
    centroids= kmeans.cluster_centers_
    return(centroids, n_clust, kmeans)

def clusterk(bboxes):
    csum=[];cdiff=[];cy=[];fac=240;maxiter=50
    for i in range(len(bboxes)):
      csum.append((bboxes[i,0]+bboxes[i,2],bboxes[i,1]+bboxes[i,3]))
      cdiff.append(bboxes[i,2]-bboxes[i,0])
      cy.append((bboxes[i,3]+bboxes[i,0])/fac)
    data=np.asarray(csum)
    minx=np.amin(np.asarray(cdiff))*2.5
    miny=(np.amax(np.asarray(cy))-np.amin(np.asarray(cy)))
    if data.shape[0]!=1:
      n_clust=int(2*np.sqrt(data.shape[0]))
    else:
      n_clust=1 
    #print(data.shape[0]," ",n_clust)
    for i in range(len(data)):
       data[i][1]=int(data[i][1]/125)
       data[i][0]=int(data[i][0])
    kmeans = KMeans(init='k-means++', max_iter=maxiter, n_clusters=n_clust, n_init=10)
    kmeans.fit(data)
    centroids= kmeans.cluster_centers_
    n_clust_=0; cnt=0
    while(n_clust!=n_clust_ and cnt<3):
       n_clust_=n_clust
       centroids, n_clust, kmeans=kmean_find(data,n_clust,centroids,minx,miny,maxiter)
       cnt=cnt+1
    label=[]
    for i in range(len(bboxes)):
      label.append(kmeans.predict(np.asarray([bboxes[i,0]+bboxes[i,2],int((bboxes[i,1]+bboxes[i,3])/125)]).reshape(1,-1)))
    labelclust=np.asarray(label).flatten()
    #print(n_clust)
    #plt.plot(centroids[:,0],centroids[:,1],'o',color='black')
    #plt.savefig("../data/cplot.jpg")
    return(labelclust)

def visualize_training_data(image_fn, bboxes, labels, clusters):
    ...
    for idx, (x1, y1, x2, y2, score) in enumerate(bboxes):
        ...
        char=str(CLASS_NAMES[labels[idx]])
...
def main():
    ...
    for idx, f in enumerate(listdir(path)):
        if isfile(join(path, f)):
            result = inference_detector(model, join(path, f))
            result = result[:-1]
...
#                 clusters = cluster(bboxes_x, eps) # if using dbscan, pls uncomment this line
                clusters = clusterk(bboxes) #kmeans
                print("Clusters: ", clusters)
                print("Box center: ", bboxes_center)
                labels = [
                ...
                cluster_dict = {}
                proposals = {}
            with open(f'{join("../data/train_jsons",f.split(".")[0])}.json', "w") as f:
                f.write(json.dumps({
                    "all_elems": all_elems,
            ...
            result=[]
    #viz = visualize_training_data(input_image, bboxes, labels, clusters)
    #plt.figure(figsize=(15, 15))
    #plt.imshow(viz, interpolation='lanczos')
    #plt.savefig("../data/output/visualclust0.jpg")
